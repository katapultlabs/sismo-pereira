...
function MedicalCard({ lang }: { lang: Lang }) {
  ...
  /* The shadow is theme-split: 40% black lifts a card off an ink ground but
     sits on bone paper like a bruise, and this card floats over the page on
     `lg`, so it needs the lift in both. */
  return (
    <div className="overflow-hidden rounded-sm border border-border bg-popover shadow-xl shadow-black/15 dark:shadow-black/40">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
...
/**
 * Desktop life safety — fixed to the viewport's bottom-right on `lg`, so it
 * stays reachable through the whole scroll. Renders nothing below `lg`;
 * `HeroSafetyMobile` carries it in the flow there.
 *
 * The 123 plate above the closures card is not decoration. On `lg` the
 * horizontal masthead — and the 123 button it carries — is hidden in favour
 * of the landing's rail, which left the hero's donate button as the only
 * large call to action on the page and 123 as a 12px row in a phone
 * directory. That inverts design principle 3 ("no appeal for money sits
 * above 'call 123'"). Fixed to the viewport it is now the one action that
 * never scrolls away, which the donate CTA does.
 */
export function HeroSafety({ lang }: { lang: Lang }) {
  const t = getDictionary(lang);

  return (
    <div className="pointer-events-none fixed right-4 bottom-4 z-40 hidden w-64 flex-col gap-2 lg:flex">
      <a
        href="tel:123"
        className="pointer-events-auto flex items-center gap-3 rounded-sm bg-down px-4 py-3 text-down-contrast shadow-xl shadow-black/25 transition-opacity outline-none hover:opacity-95 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
      >
        <Phone className="size-4 shrink-0" aria-hidden />
        <span className="min-w-0 flex-1">
          <span className="label-signage block text-[0.5625rem] opacity-80">
            {t.actions.emergency.eyebrow}
          </span>
          <span className="display-condensed block text-lg leading-none font-extrabold uppercase">
            {t.actions.emergency.title}
          </span>
        </span>
      </a>
